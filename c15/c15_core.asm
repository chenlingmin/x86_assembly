        ...
        mem_0_4_gb_seg_sel      equ 0x08        ;整个0-4GB内存段选择子
...
read_hard_disk_0:                               ;从硬盘读取一个逻辑扇区
        ...
        mov cl, 8
        shr eax, cl
        out dx, al                              ;LBA地址的15~8

        inc dx                                  ;0x1f5
        shr eax, cl
        out dx, al                              ;LBA地址的23~16

        inc dx                                  ;0x1f6
        shr eax, cl
        or  al, 0xe0                            ;第一硬盘 LBA地址的27~24
        out dx, al
...
load_relocate_program:                          ;加载并重定位用户程序
        ...
        add ebx, 512
        test eax, 0x000001ff                    ;程序的大小正好是512的倍数么？
        ...
        mov eax, [ebp+12*4]                     ;起始扇区段
    .b1:
        call sys_routine_seg_sel:read_hard_disk_0
        inc eax
        loop .b1                                ;循环读，直到读完整个用户程序

        mov edi, [es:esi+0x06]                  ;获得程序加载的基地址
    ...
    .b3:
        ...
        mov ax, [esi+4]
        or  ax, 0000_0000_0000_0011B            ;以用户程序自己的特权级使用调用门
                                                ;故RPL=3
        mov [es:edi-252], ax                    ;改写段选择子
    .b4:
        ...
        mov [es:esi+0x1a], ecx
        shr dword [es:esi+0x1a], 12             ;登记0特权级堆栈尺寸到TCB(4096)
        call sys_routine_seg_sel:allocate_memory
        add eax, ecx                            ;堆栈必须使用高地址为基地址
        ...
        mov ebx, esi                            ;TCB的基地址
        call fill_descriptor_in_ldt
        or cx, 0000_0000_0000_0000              ;设置选择子的特权为0
        ...
        mov eax, ecx                            ;为生成堆栈高端地址做准备
        mov [es:esi+0x28], ecx
        shr dword [es:esi+0x28], 12             ;登记1特权级堆栈尺寸到TCB(4096)
        call sys_routine_seg_sel:allocate_memory
        add eax, ecx                            ;堆栈必须使用高地址为基地址
        ...
        mov ebx, esi                            ;TCB的基地址
        call fill_descriptor_in_ldt
        or cx, 0000_0000_0000_0001              ;设置选择子的特权为1
        ...
        mov eax, ecx                            ;为生成堆栈高端地址做准备
        mov [es:esi+0x36], ecx
        shr dword [es:esi+0x36], 12             ;登记2特权级堆栈尺寸到TCB(4096)
        call sys_routine_seg_sel:allocate_memory
        add eax, ecx                            ;堆栈必须使用高地址为基地址
        ...
        mov ebx, esi                            ;TCB的基地址
        call fill_descriptor_in_ldt
        or cx, 0000_0000_0000_0010              ;设置选择子的特权为2
...
start:
    ...
    .b3:
        ...
        call far [es:ecx+0x14]                  ;执行任务切换。和上一章不同，任务切换时要恢复TSS内容，所以在创建时TSS要填完整
