        ...
        ;创建保护模式下初始代码段描述符
        mov dword [ebx+0x10], 0x7c0001ff    ;基地址为 0x00007c00，界限为 0x1FF
        mov dword [ebx+0x14], 0x00409800    ;粒度为1字节，代码段描述符

        ;建立保护模式下的堆栈段描述符
        mov dword [ebx+0x18], 0x7c00fffe    ;基地址为 0x00007c00，界限为 0xFFFFE
        mov dword [ebx+0x1c], 0x00cf9600    ;粒度为4KB

        ;建立保护模式下的显示缓冲区描述符
        mov dword [ebx+0x20], 0x80007fff    ;基地址为 0x0000B800，界限为 0x07FFF
        mov dword [ebx+0x24], 0x0040920b    ;粒度为1字节

        ;初始化描述符表寄存器GDTR
        mov word [cs: pgdt+0x7c00], 39      ;描述符表的结项
    ...
    flush:
        ...
        mov ebx, edi                        ;起始地址
        call read_hard_disk_0               ;一下读取程序的起始部分(一个扇区)
    ...
    setup:
        ...
        mov word [0x7c00+pgdt], 63          ;描述符的界限
