...
;=================================================================================
SECTION code align=16 vstart=0              ;定义代码段
...
new_int_0x70:
    ...
    .w0:
        ...
        jnz .w0                             ;以上代码对于更新周期结束中断来说，是不必要的

        xor al, al
        or  al, 0x80
        out 0x70, al
        in  al, 0x71                        ;读取RTC当前时间 秒
        push ax

        mov al, 2
        or  al, 0x80
        out 0x70, al
        in  al, 0x71                        ;读取RTC当前时间 分
        push ax

        mov al, 4
        or  al, 0x80
        out 0x70, al
        in  al, 0x71                        ;读取RTC当前时间 时
        push ax

        mov al, 0x0c                        ;寄存器C的索引，开放 NMI
        out 0x70, al
        in  al, 0x71                        ;读一下RTC寄存器C，否则只发生一次中断
                                            ;此处不考虑闹钟和周期性中断的情况
...
;---------------------------------------------------------------------------------
    start:
        ...
        mov al, 0x70
        mov bl, 4
        mul bl                              ;计算0x70号中断在中断向量表（IVT）的偏移
        mov bx, ax
