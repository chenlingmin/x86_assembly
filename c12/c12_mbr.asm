        ...
        ;创建#1描述符，这是一个数据段，对应0~4G的线性地址空间
        mov dword [ebx+0x08], 0x0000ffff    ;基地址为0，段界限为 0xffff
        mov dword [ebx+0x0c], 0x00cf9200    ;粒度为 4KB，存储器段描述符

        ;创建保护模式下初始代码段描述符
        mov dword [ebx+0x10], 0x7c0001ff    ;及地址为0x00007c00, 512字节
        mov dword [ebx+0x14], 0x00409800    ;粒度为1字节，代码段描述符

        ;创建以上代码段的别名描述符
        mov dword [ebx+0x18], 0x7c0001ff    ;及地址为0x00007c00, 512字节
        mov dword [ebx+0x1c], 0x00409200    ;粒度为1字节，数据段描述符
